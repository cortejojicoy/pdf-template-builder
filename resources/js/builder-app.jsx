...
// Mounts the BuilderView, wires the save/upload API calls, and shows
// the Filament-style top bar with real save state.
...
(function () {
  ...
  function App() {
    ...
    // Trigger unsaved indicator on any field change
    ...
    // Undo/Redo via browser history state (lightweight)
    ...
    return (
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)', color: 'var(--text)' }}>
        <BuilderTopBar
          ...
          listUrl={LIST_URL}
          onSave={() => {
            // BuilderView will call handleSave via onSave prop
            window.__builderSave && window.__builderSave();
          }}
        />
      ...
      <header style={{
        ...
        <button onClick={onSave} disabled={saving}
          style={{
            ...
            fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6,
            boxShadow: '0 1px 2px rgba(0,0,0,.06), inset 0 1px 0 rgba(255,255,255,.1)',
